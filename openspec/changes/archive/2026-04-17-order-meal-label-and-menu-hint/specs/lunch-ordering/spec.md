...
The system SHALL allow the organizer to create a new ordering session by providing a restaurant name via the `/新增訂單 <restaurant>` command. After saving the new order, the system SHALL enter a `setting_meal_label` wizard state and prompt the organizer to optionally enter a meal label (free text, e.g., "明天午餐", "4/18 便當"). After the meal label step (whether set or skipped), the wizard SHALL transition to the `setting_deadline` state and prompt the organizer to optionally enter an order deadline (free text, e.g., "4月18號中午12點"). After the deadline step (whether set or skipped), the wizard SHALL transition to the `setting_threshold` state and prompt the organizer to optionally enter a delivery threshold amount. After the threshold step (whether set or skipped), the wizard SHALL transition to the `adding_items` state and prompt the organizer to submit all menu items in a single multi-line message. Each line in the message SHALL represent one menu item in the format `品名 金額`. If all lines are valid, the system SHALL display the parsed items back to the organizer for confirmation and transition to the `confirming_menu` state. If any line is malformed, the system SHALL reject the entire batch, list the malformed lines in the reply, and remain in `adding_items` for the organizer to retry. In the `confirming_menu` state, if the organizer sends `/確認`, the system SHALL save all pending items and send the menu Flex Message to the group, ending the wizard. If the organizer sends any other multi-line message, the system SHALL re-parse it as a new batch, replace the pending items, and display the new preview for confirmation. Only one active order (status `open`) SHALL exist at a time; attempting to create a new one while another is active SHALL be rejected with an error message.

The meal label SHALL be stored in the `orders.meal_label` column (TEXT, nullable). If the organizer sends `/略過` during the `setting_meal_label` state, `meal_label` SHALL be stored as NULL.

The order deadline SHALL be stored in the `orders.order_deadline` column (TEXT, nullable). If the organizer sends `/略過` during the `setting_deadline` state, `order_deadline` SHALL be stored as NULL. The system SHALL NOT validate or parse the deadline text; it is stored and displayed as-is.

#### Scenario: Organizer creates a new order

- **WHEN** organizer sends `/新增訂單 老媽便當` in the group
- **THEN** the system SHALL save a new order with restaurant_name "老媽便當" and status `open`, set the session state to `setting_meal_label`, and prompt the organizer to enter a meal label or `/略過`

#### Scenario: Organizer enters a meal label

- **WHEN** organizer sends a plain text message during the `setting_meal_label` state (e.g., "明天午餐")
- **THEN** the system SHALL store the text as `meal_label` on the order, transition the session to `setting_deadline`, and prompt the organizer to enter a deadline or `/略過`

#### Scenario: Organizer skips meal label

- **WHEN** organizer sends `/略過` during the `setting_meal_label` state
- **THEN** the system SHALL store NULL as `meal_label` on the order, transition the session to `setting_deadline`, and prompt the organizer to enter a deadline or `/略過`

#### Scenario: Organizer enters a deadline

- **WHEN** organizer sends a plain text message during the `setting_deadline` state (e.g., "4月18號中午12點")
- **THEN** the system SHALL store the text as `order_deadline` on the order, transition the session to `setting_threshold`, and prompt the organizer to enter a delivery threshold or `/略過`

#### Scenario: Organizer skips deadline

- **WHEN** organizer sends `/略過` during the `setting_deadline` state
- **THEN** the system SHALL store NULL as `order_deadline` on the order, transition the session to `setting_threshold`, and prompt the organizer to enter a delivery threshold or `/略過`
