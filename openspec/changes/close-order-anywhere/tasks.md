...
- [ ] 3.1 手動測試：主辦人在 wizard 輸入品項階段（`adding_items`）傳送 `/關閉訂單`，確認訂單關閉、session 清除、回覆確認訊息
- [ ] 3.2 手動測試：主辦人在非 wizard 狀態（open 訂單）傳送 `/關閉訂單`，確認行為與修改前相同
- [ ] 3.3 手動測試：非主辦人傳送 `/關閉訂單`，確認收到權限錯誤，訂單不關閉
